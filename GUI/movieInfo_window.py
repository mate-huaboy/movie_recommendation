...
class movieinfo_window():
    ...
    def __init__(self,root,window,movieid,userid):
        ...
        self.root = root
        """基于功能的推荐器产生"""
        #self.reconFunc=recommendBasedOnfunc(self.userid)
        #self.reconFunc=reconFunc
        ...
        GUI.metaButtons.NavigationBar(self.root, self.window, navbar_Frame, self.movieid, self.userid, type="I")
        ...
        self.title, self.date, self.genres, self.briefinfo = metaFun_GUI.get_src(self.movieid)
        self.tk_image=metaFun_GUI.get_image(self.movieid,w_box=300,h_box=300)
        ...
        tk.Label(self.window, text="相似电影：").place(x=25, y=445, anchor="nw")
        # self.data_SVD = metaFun_GUI.get_similar_movie_list(self.movieid,type="SVD")
        data=GUIGlobalVar.r.recmProduct(self.movieid,10)
        ...
        for tup in data:
            k=k+1
            if k<=5:
                t = threading.Thread(target=self.job, args=(tup, self.frm_svd))
                t.start()
            else:
                t = threading.Thread(target=self.job, args=(tup, self.frm_als))
                t.start()

    def get_score(self):
        #获取评分数据
        conn,cur = GlobalFun.ConnectSql()
        sql = "select score,times from dbmovierecommender.movie_score_info where movieid={};".format(self.movieid)
        cur.execute(sql)
        data = cur.fetchall()
    ...
    def job(self,movieid,frm):
        temp = GUI.metaFrame.metaFrame(movieid, self.userid, frm, self.window, self.root)
